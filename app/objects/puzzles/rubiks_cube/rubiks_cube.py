from primitives.cube.cube import Cube
from itertools import product as pd
from app.viewport.view import device_data, fm
from app.objects.puzzles.rubiks_cube.data import color_map
...
class RubiksCube:
    ...
    def queue_init(self):
        for corner in self.__corners:
            self.queue.append(Cube(self.camera, self.pg, corner[1], corner[0]))

        for edge in self.__edges:
            for i in range(len(edge[0])):
                self.queue.append(Cube(self.camera, self.pg, edge[1], edge[0][i]))

        for center in self.__centers:
            for i in range(len(center[0])):
                self.queue.append(Cube(self.camera, self.pg, center[1], center[0][i]))
