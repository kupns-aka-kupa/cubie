...
from app.viewport.view import device_data, fm
key_map = fm.open_file('app/settings/key_bindings/key_map.json')


class Events:
    def __init__(self, viewport):
        self.__viewport = viewport
        self.__camera = viewport.camera
        self.__pg = viewport.pg
        self.shift = False
        self.ctrl = False
    ...
    def update(self):
        for event in self.__pg.event.get():
            if event.type == self.__pg.QUIT: self.__pg.quit(); sys.exit()
    ...
    def keyboard_keydown(self, event):
        ...
        if event.key == key_map.get('o'):
            self.__camera.orto_view = not self.__camera.orto_view

        if event.key == key_map.get('-'):
            self.__camera.pos[2] += 1

        if event.key == key_map.get('+'):
            self.__camera.pos[2] -= 1

        if event.key == key_map.get('U'):
            if self.shift:
                U_angle -= math.pi / 2
            else:
                U_angle += math.pi / 2

        if event.key == key_map.get('D'):
            if self.shift:
                D_angle -= math.pi / 2
            else:
                D_angle += math.pi / 2

        if event.key == key_map.get('R'):
            if self.shift:
                R_angle -= math.pi / 2
            else:
                R_angle += math.pi / 2

        if event.key == key_map.get('L'):
            if self.shift:
                L_angle -= math.pi / 2
            else:
                L_angle += math.pi / 2

        if event.key == key_map.get('F'):
            if self.shift:
                F_angle -= math.pi / 2
            else:
                F_angle += math.pi / 2

        if event.key == key_map.get('B'):
            if self.shift:
                B_angle -= math.pi / 2
            else:
                B_angle += math.pi / 2
        ...
        self.__viewport.rubiks_cube.logic(angles)

    def mouse_move(self, event):
        self.__viewport.gui.mouse_x, self.__viewport.gui.mouse_x = event.pos
        if self.middle_mouse:
            x, y = event.rel
            self.__camera.rot[0] += y / device_data['sensivity']
            self.__camera.rot[1] += x / device_data['sensivity']
