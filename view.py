import tools; import cube; import grid; import data; import os
tools = tools.Tools()
data = data.Data()
colors = data.colors
...
#            self.rubicube.append(self.cube.Cube(self.camera, self.pg, data, tools, i[0]))
        ...
        for edge in self.edges :
            for i in  range(len(edge[0])):
                self.rubicube.append(self.cube.Cube(self.camera, self.pg, data, tools, edge[0][i]))
                self.rubicube_colors.append(edge[1])

    def display_order(self):
        order = []
        for i in self.rubicube:
            rot = (abs(tools.math.asin(i.x0 / tools.math.sqrt(i.y0**2 + i.x0**2)) - self.camera.rot[0]), abs(tools.math.asin(i.z0 / tools.math.sqrt(i.y0**2 + i.z0**2)) - self.camera.rot[0]))
            order.append(rot)
        return sorted(range(len(order)), key = lambda i : order[i])

    def update(self):
        self.grid.render()
        order = self.display_order()
        for i in range(len(self.rubicube)):
            self.rubicube[order[i]].render( self.rubicube_colors[order[i]])
